  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8" id="detalhes-moderar">{selectedPost ? <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm space-y-6"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4"><div><div className="flex items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-wider text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-0.5 rounded-full">Pendente Homologação</span><span className="text-[10px] text-slate-400 font-mono">ID: {selectedPost.id}</span></div><h3 className="text-lg font-bold text-slate-800 mt-1.5">{selectedPost.titulo}</h3><div className="mt-1 flex flex-wrap gap-2 text-[10.5px] text-slate-500"><span className="flex items-center gap-1"><User className="w-3 h-3 text-slate-400" /> Cadastrado por: <strong className="text-slate-700 font-medium">{selectedPost.criado_por_nome || currentUser.nome}</strong></span><span className="text-slate-300">•</span><span>Criado em: {new Date(selectedPost.criado_em).toLocaleString('pt-BR')}</span></div></div><div className="flex gap-2"><input id="replace-post-media-input" type="file" accept="image/*,video/mp4,video/quicktime,video/webm" onChange={(e) => { const file = e.target.files?.[0]; if (file) void replaceMedia(file); e.currentTarget.value = ''; }} className="hidden" /><label htmlFor="replace-post-media-input" className="p-2 sm:px-3 text-xs font-semibold text-slate-700 bg-slate-55 border border-slate-200 rounded-lg flex items-center gap-1.5 shrink-0 cursor-pointer">{uploadingMedia ? <Loader2 className="w-4 h-4 animate-spin" /> : <UploadCloud className="w-4 h-4" />}<span className="hidden sm:inline">{uploadingMedia ? 'Enviando Mídia' : 'Trocar Mídia'}</span></label><button onClick={() => void openEditor()} disabled={!canEditImage} className="p-2 sm:px-3 text-xs font-semibold text-slate-700 bg-slate-55 border border-slate-200 rounded-lg flex items-center gap-1.5 shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"><Scissors className="w-4 h-4" /><span className="hidden sm:inline">Editar Imagem</span></button><button onClick={() => setIsEditing(!isEditing)} className="p-2 sm:px-3 text-xs font-semibold text-slate-700 bg-slate-55 border border-slate-200 rounded-lg flex items-center gap-1.5 shrink-0"><Edit3 className="w-4 h-4" /><span className="hidden sm:inline">{isEditing ? 'Cancelar Edição' : 'Editar Legenda'}</span></button></div></div>
